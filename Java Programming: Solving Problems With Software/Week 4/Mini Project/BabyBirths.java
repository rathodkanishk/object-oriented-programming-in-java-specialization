        ...
        for (File f : dr.selectedFiles()) {
            ...
            for (CSVRecord rec : fr.getCSVParser(false)) {
                ...
                if (rec.get(0).equals(name) && rec.get(1).equals(gender)) {                    
                    currRank = getRank(yob, name, gender);

                }                
            }
            // If rank is -1 or if currRank is less than rank, update rank to be currRank and update year to yob
            if (rank == -1 || currRank < rank) {
                rank = currRank;
                year = yob;
            }
        }
